...
const Navbar = () => {
  ...
  return (
    <nav className='flex-between w-full mb-16 pt-3'>
      ...
      <Image src="/assets/images/logo.svg" alt="main logo" width={30} height={30} className='object-contain'/>
      <p className='logo_text'>Test App</p>
      </Link>
    ...
    {providers && Object.values(providers).map(provider=>(<button type='button' key={provider.name} onClick={()=> signIn(provider.id)} className='black_btn'>Sign In</button>))}
      ...
      <div className='sm:hidden flex relative'>
        {isUserLoggedIn ? <div className='flex'>
          <Image src="/assets/images/logo.svg" width={37} height={37} className='rounded-full' alt="profile" onClick={()=>{setToggleDropdown(prevState=> !prevState)}} />

          {toggleDropdown && (<div className='dropdown'>
            <Link href="/profile" className='dropdown_link' onClick={()=>setProviders(false)}>My Profile</Link>
            <Link href="create-prompt" className='dropdown_link' onClick={()=>setProviders(false)}>Create Prompt</Link>
